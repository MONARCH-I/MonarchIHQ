...
Route::get('/manager', function () {
    $user = auth()->user();
    if (!$user) return redirect()->route('manager.login');
    if ($user->isSuperAdmin() || ($user->isContentManager() && $user->role === 'content_manager')) {
        return redirect()->route('manager.content.index');
    }
    if ($user->role === 'store_manager') return redirect()->route('manager.store.index');
    if ($user->role === 'hr_manager')   return redirect()->route('manager.hr.index');
    abort(403);
