<x-manager-sidebar>
    ...
    <x-slot name="sidebarNav">
        ...
        <a href="{{ route('manager.hr.applications') }}" class="sidebar-nav-link {{ request()->is('manager/hr/applications*') ? 'active' : '' }}">
            ...
            <span class="badge" style="background:#9c051d;color:#fff;border:none;padding:1px 6px;font-size:9px;margin-left:auto">{{ $stats['pending_applications'] }}</span>
            @endif
        </a>
        <a href="{{ route('manager.hr.messages') }}"     class="sidebar-nav-link {{ request()->is('manager/hr/messages*') ? 'active' : '' }}">
            <span>✉️</span> Messages
            @if($stats['new_messages'] > 0)
            <span class="badge" style="background:#9c051d;color:#fff;border:none;padding:1px 6px;font-size:9px;margin-left:auto">{{ $stats['new_messages'] }}</span>
            @endif
        </a>
    </x-slot>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:16px;margin-bottom:28px">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value" style="color:#9c051d">{{ $stats['total_applications'] ?? 0 }}</div>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value" style="color:#9c051d">{{ $stats['new_messages'] }}</div>
        </div>
        <div class="stat-card">
    ...
    <div class="card">
        ...
        <a href="{{ route('manager.hr.messages.show', $msg) }}" style="display:block;padding:12px 14px;background:var(--bg-hover);border-radius:10px;border:1px solid var(--border);text-decoration:none;transition:border-color 0.15s" onmouseover="this.style.borderColor='rgba(156,5,29,0.3)'" onmouseout="this.style.borderColor='var(--border)'">
